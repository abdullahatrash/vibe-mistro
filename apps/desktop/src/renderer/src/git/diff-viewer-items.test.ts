...
describe('fnv1a32 / buildPatchCacheKey', () => {
  it('is deterministic and content-sensitive', () => {
    expect(fnv1a32('abc')).toBe(fnv1a32('abc'))
    expect(fnv1a32('abc')).not.toBe(fnv1a32('abd'))
  })

  it('folds patch length + content into a stable key, distinct per patch', () => {
    ...
    expect(buildPatchCacheKey(patch, 'scope-x')).toMatch(/^scope-x:/)
  })
})
