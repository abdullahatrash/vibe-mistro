...
// DEFAULT userData dir, so the existing profile dir is pinned FIRST — otherwise the
// rename would silently orphan every stored Workspace/Thread/transcript under a
// fresh "Vibe Mistro (Beta)" dir. (The e2e seam below still wins — it runs after.)
app.setPath('userData', join(app.getPath('appData'), 'vibe-mistro'))
...
// sidebar Logo's vector data). macOS ignores BrowserWindow `icon` — its Dock icon is
// set via `app.dock.setIcon` at ready (dev; a packaged bundle carries icon.icns).
        ...
        onOpenSettings: () => {
          for (const win of BrowserWindow.getAllWindows()) {
            if (!win.webContents.isDestroyed()) {
              win.webContents.send(IPC.menuAction, { action: 'open-settings' })
            }
          }
        },
      })
