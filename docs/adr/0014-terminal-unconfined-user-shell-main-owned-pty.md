...
Terms: `CONTEXT.md` **Terminal session**. Reference shape: a production terminal manager + drawer
(server-owned PTY behind an attach-stream; the UI a disposable view).

## Context

The feature roadmap reserves a Terminal card in the side panel (#187/#193 shipped it inert). The
production GUI we studied ships the target shape: xterm.js as a thin, reconnectable view; the PTY
process, its scrollback, and its lifetime owned by a backend server; plain UTF-8 strings over the
wire; client-chosen session ids. We have no server process — the Electron main process IS our backend
— and our side panel is per-Workspace (the reference's is per-Thread; our Threads share the working
tree).
...
2. **A FULL, UNCONFINED user shell — deliberately.** CLI parity: the same shell, env, and reach the
   user has in their own terminal (they could run anything there anyway; production terminal
   drawers ship the same posture). This is the OPPOSITE of the confined `files:*` reads (ADR-0013) and that contrast is
...
3. **Per-Workspace sessions, independent of the warm agent.** One session per Workspace this slice
   (id `term-1`, client-chosen; the event shape already carries `terminalId`
...
4. **The view reattaches; the session buffers.** Tab switches and panel closes unmount xterm; the
   ...
   (a shell is ephemeral; disk-persisted history files are deferred with the rest of slice-2+ polish).
...
5. **Packaging: no rebuild.** node-pty 1.x is N-API/ABI-stable — no `electron-rebuild`, no
   postinstall (verified against a production Electron manifest). node-pty is imported ONLY in the
...
Output **coalescing / sequence-numbered attach** (the reference has both) is deliberately NOT ported: our
...
A session is now keyed by `(workspaceId, terminalId)`, so a Workspace
...
- **Live re-theme** — the xterm theme is computed CSS read at mount; a `MutationObserver` on the root's
  `class`/`style` re-reads it on a dark/light flip, so an open terminal doesn't
  strand stale colors.
