...
vibe-mistro sits a layer above the agent, as orchestrator GUIs sit above the CLI agents they
drive. When Vibe uses code intelligence, it reaches us as ACP tool-call items that we simply display.
...
- **Own an LSP subsystem from the start (opencode-style)** — rejected. Wrong layer: it duplicates
  ...
  plumbing), and serves none of the planned slices. Comparable orchestrator GUIs ship no LSP and
  are complete.
