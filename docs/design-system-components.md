...
build issues **#110–#119**. This captures the reusable findings from the reference-GUI + shadcn/ui
explorations so the build session doesn't re-derive them. **Copy-adapt and own in-repo; never add the
references as deps.**
...
- **reference chat GUI** — a production multi-agent chat app we studied locally (chat rows,
  markdown renderer, and `ui/` primitives).
- **prototype** — `/Users/abdullahatrash/mistral/Vibe Mistro.html` (exact tokens/layout).

Both references use **our stack** (Tailwind v4 + `@base-ui/react` + CVA + `cn()` + lucide), so the
...
## 2. Conversation (issues #114–#116) — mine the chat reference, structure from shadcn

**Architecture (keep ours):** both refs use a discriminated-union + switch, NOT deep compound components —
the chat reference = row `kind` union + one timeline-row dispatcher (+ React Context for shared state);
...
`react-markdown` + `remark-gfm`/`remark-breaks` + shiki (the reference's chat-markdown component is the
blueprint — but its authors called their shiki+LRU-cache layer their "most over-engineered piece",
which streamdown packages).
- **File-path links** (the standout — the reference's markdown file-link component): resolve `href`/path →
...
- The reference's markdown styling is a **self-contained global CSS block** (`.chat-markdown-*`,
  theme-var driven) — copyable if not using streamdown's styling.
...
**ToolRow** (#115 — the reference's compact work-entry row): compact `flex items-center gap-1.5
...
**Reasoning** (#115): a **Collapsible** "thinking" block, **auto-open while streaming**. (The chat
reference renders reasoning as a dimmed tool-like row; shadcn renders a flat shimmer — we do the
collapsible auto-open per ADR-0010.)

**Working indicator** (#115): a working-timeline row — three pulsing dots (`h-1 w-1 rounded-full
bg-muted-foreground/30 animate-pulse` staggered `[animation-delay:200ms/400ms]`) + a **self-ticking**
"Working for 12s" label (a timer that updates `textContent` via `setInterval`, no React re-render). Copy
this pattern.

**Approval** (#116 — kept **INLINE**, unlike references that put it in the composer footer): restyle our
existing `PermissionRow`. Map buttons to OUR ACP permission options (allow-once / reject-once / etc.).
A **numbered-option keyboard panel** (1–9 kbd chips, auto-advance,
selected `border-primary/30 bg-primary/8`) is a nice pattern if we later render option prompts.

**Actions bar** (#116): hover-reveal (`opacity-0 group-hover:opacity-100 transition-opacity`) — **copy**
(icon→check + **anchored toast** on the button, not inline — a proven message-copy-button pattern) +
**thumbs up/down** + **retry** (we add thumbs/retry per the mockup; the reference only had copy+revert).
Assistant copy hidden while streaming.

**Strip from the chat reference** (its coupling — do NOT port): worktree/checkpoint diff summaries,
revert/"revert to message", proposed-plan cards, terminal/element/preview-annotation contexts, skills
inline, **Pierre file icons** (use lucide), `localApi`/editor-open plumbing in file links, its
session-logic/timeline derivation layer (replace with our ACP reducer output), and
